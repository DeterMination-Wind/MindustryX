...
- 首次拉仓库或子模块缺失时，先执行 `git submodule update --init --depth=10`，再处理 patch 展开或构建。
...
- `patches/` 改动应尽量最小化；能在 `src/` 或展开源码实现的，不要为了方便额外扩大 patch 面。
- 同一功能的 patch 应尽量合并在现有 patch 里，不要为一个连续功能点再拆出新的零散 patch。
...
- `i(...)` 的返回值不允许再参与字符串拼接；需要变量插值或复杂组合时，改成完整 bundle 词条或结构化 UI 组件。
